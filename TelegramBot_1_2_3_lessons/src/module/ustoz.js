...
const Users = new Io("./database/ustozlar.json")


const router = new Router((ctx) => ctx.session.step)

const sherikkk = router.route('ustoz')
...
sherikkk.on("message:text", async(ctx) =>{
    ...
    ctx.session.step = "ubir"
})

const bir = router.route('ubir')
...
bir.on("message:text", async(ctx) =>{
    ...
    ctx.session.step = "uikki"
})

const ikki = router.route('uikki')
...
ikki.on("message:text", async(ctx)=>{
    ...
    ctx.session.step = "uuch"
})

const uch = router.route('uuch')
...
uch.on("message:text", async(ctx)=>{
    ...
    ctx.session.step = "uturt"
})

const turt = router.route('uturt')
...
turt.on("message:text", async(ctx)=>{
    ...
    ctx.session.step = "ubesh"
})

const besh = router.route('ubesh')
...
besh.on("message:text", async(ctx)=>{
    ...
    ctx.session.step = "uolti"
})

const olti = router.route('uolti')
...
olti.on("message:text", async(ctx)=>{
    ...
    ctx.session.step = "uyetti"
})

const yetti = router.route('uyetti')
...
yetti.on("message:text", async(ctx)=>{
    ...
    ctx.session.step = "usakkiz"
})


const sakkiz = router.route('usakkiz')
...
sakkiz.on("message:text", async(ctx)=>{
    ...
    ctx.session.step = "u10"
})

const sh10 = router.route('u10')
