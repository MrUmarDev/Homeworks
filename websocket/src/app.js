    ...
    socket.on('createGroup', async (data) => {
        ...
        if (!findAuthor) {
            ...
            if (!findGroup) {
                ...
                socket.to('Chat1').emit('new-message', {message: `${data.authorName} joined to the group ${data.groupName}`})
            }
            ...
            if (!findGroup) {
                await createGroup(data.groupName, data.authorName)
                socket.join(data.groupName)
                socket.to('Chat2').emit('new-message', {message: `${data.authorName} joined to the group ${data.groupName}`})
            }
            info = await Groups.read()
            io.emit('groups', {info})
        }
